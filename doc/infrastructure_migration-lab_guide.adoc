...
. As far as migration goes, `hana` will be migrated and powered on first, since it has already been running RHEL in the source environment. 
The other two machines, `tomcat` and `oracledb` will need more time. The longest migration time is needed by the most complex VM deployment, and that is OracleDB running on Oracle Linux. + 
