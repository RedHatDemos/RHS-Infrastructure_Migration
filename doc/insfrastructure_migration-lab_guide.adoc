...
** `jboss0` - a Red Hat Enterprise Linux 7 host running JBoss EAP, connected to the `Service` network.
** `jboss1` - a Red Hat Enterprise Linux 7 host running JBoss EAP, connected to the `Service` network.
** `lb` - a Red Hat Enterprise Linux 7 host running JBoss Core Service Apache HTTP server configured with mod_cluster to proxy traffic to `jboss0` and `jboss1`, connected to the `Service` and `Servicer-DMZ` networks.
** `db` - a Red Hat Enterprise Linux 7 host running PostgreSQL providing service to `jboss0` and `jboss1` through the `Service` network.
