...
.. Download the CSV file from link:https://github.com/RedHatDemos/RHS-Infrastructure_Migration/blob/master/scripts/ticket_monster_migration.csv[here]. And check that the content is the following:
+
...
.. Navigate to **Compute**, **Migration** and choose **Create Migration Plan**. We will use the same infrastructure mapping previously created.
+
image::multi_vm_migration_plan_1.png[Multiple VM Migration Plan 1]

.. Skip pre/post playbooks and select the remaining VMs for migration
+
image::multi_vm_migration_plan_2.png[Multiple VM Migration Plan 2]

.. Select **Start the migration plan immediately** and clock **Create**.
+
