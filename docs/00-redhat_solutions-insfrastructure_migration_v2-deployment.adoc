...
The environment is completely configuredi, and an overview look at it, is recommended before starting.
...
The file `inventory.example.yml` is created, in the `v2v-transformation_host-ansible` directory, with the following content:
...
# ansible-playbook --inventory-file=inventory.example.yml transformation_host_check.yml 
----
...
----
# cd /root/v2v-transformation_host-ansible
# ansible-playbook --inventory-file=inventory.example.yml transformation_host_enable.yml 
----
