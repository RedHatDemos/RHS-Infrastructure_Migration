...
The following https://github.com/RedHatDemos/RHS-Optimize_IT-Infrastructure_Migration/blob/master/notes_v2/rhv_infra_migration.repo[repo file] is used to consume the packages in the environment for the Manager, as well as for the Hypervisors, which are RHEL based.
...
Once the environment has ben reset to a just installed state, the Virtualization providers have to be added to CloudForms. It can be done by following these steps:
...
We will use both hypervisors, `kvm0` and `kvm1`, as conversion hosts. We will only install `kvm1`.
...
* The process was done with host `kvm0` already.
