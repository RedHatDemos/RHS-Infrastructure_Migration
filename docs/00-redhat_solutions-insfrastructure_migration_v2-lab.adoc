...
This Lab Environment uses ManageIQ/Red Hat CloudForms, and the Infrastructure Migration plugin, as a broker to migrate VMs from VMware vSphere to Red Hat Virtualization. 
...
+
[NOTE]
A login page may appear. Introduce the credentials used to login in ManageIQ/CloudForms (fixed in future release)
+
...
+
[TIP]
To get the VM ids can also be obtained from a web browser by accessing the following CloudForms URL (remember to change the GUID):  https://cf-<YOUR-GUID>.rhpds.opentlc.com/api/vms?expand=resources
