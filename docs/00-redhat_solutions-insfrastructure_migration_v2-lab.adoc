...
. Under *All Services -> Red Hat Solutions*, select *Infrastructure Migration Demo v2*.
+
[NOTE]
...
* The process may be repeated with host `kvm1`
...
. In a different brower page, download the CSV file with the VMs link:https://raw.githubusercontent.com/RedHatDemos/RHS-Optimize_IT-Infrastructure_Migration/master/notes_v2/ticket_monster_migration.csv[ticket_monster_migration.csv]
