...
. Under *All Services -> Migration*, select *Batch_Pre_Migrate*.

. On the right, click *Order*.

. For *Migration Group*, select `demo2` and click *Submit*.
...
== Migrate the jboss VMs from VMware to RHV Platform
...
. Select *Point of Arrival* and check that *RHV* is the assigned value.
+
...
. For *Migration Group*, select `demo1` then click *Submit*.
