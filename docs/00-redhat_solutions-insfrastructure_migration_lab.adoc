...
. Use CloudForms to shut down (_not_ power off) the VMs to be migrated. Initially just `db` but, all VMs can be shut down (`db`, `jboss0`, `jboss1`, `nginx`).
+
[TIP]
...
For all application VMs (`db`, `jboss0`, `jboss1`, `nginx`) the check command is:
`# ansible app -m ping`
...
. On the resulting screen, enter `attribute_db.csv` in the *Filename* field and click *Submit*.
...
. On the resulting screen, enter `tag_db.csv` in the *Filename* field and click *Submit*.
...
. Use CloudForms to shut down (_not_ power off) the VMs to be migrated. Initially just `jboss0` and `jboss1` but, all VMs can be shut down (`db`, `jboss0`, `jboss1`, `nginx`).
+
[TIP]
...
* Use what you learned in this lab to migrate `nginx` to RHV Platform (Tip: use `demo3` migration group)

== Extra Tip

* You can use `attribute_all.csv` for attributes, `tag_all.csv` for tags, and `fullmigration` as migration group, to perform a full migration.
